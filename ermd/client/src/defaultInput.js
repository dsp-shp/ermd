const defaultInput = `<!--
Привет, это инструмент для отрисовки ER-диаграмм на Markdown-like языке.
...
Далее задаем списком атрибуты через "-". Каждая строка состоит:
...
Порядок задания параметров произвольный, а типы и ключи не чувствительны к регистру:
–-->
...
Заметьте, тут отрисовывается отдельная сущность "test_rels" с указанным атрибутом "id", несмотря на то, что далее она нигде явно не задана. Такая сущность получает тип "UNDEFINED" и тип атрибута "UNKNOWN", что говорит о необходимости ее объявления.

Также оба атрибута автоматически получают по внешнему ключу.
–-->
...
Далее перечислим все связи отдельно от их сущностей в формате
схема1.таблица1---(Тип связи)---схема2.таблица2
–-->
* fact_sales.product_id---(0..M:1)---dim_product.product_id
* fact_sales.date_id---(M:1 "Тестовое описание для связи")---dim_date.date_id
...
<!--
Теперь можете стереть это все и попробовать отрисовать свою диаграмму.
–-->`;
